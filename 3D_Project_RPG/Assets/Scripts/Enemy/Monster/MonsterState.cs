...
public class MonsterAttackState : MonsterBaseState
{
    ...
    public override void Update()
    {
        ...
        if (monster.target == null || Vector3.Distance(monster.transform.position, monster.target.position) > monster.nav.stoppingDistance + 3f)
        {
            controller.ChangeState(controller.ChaseState);
            return;
        }
        ...
        if (attackTimer >= attackCooldown)
        {
            ...
            if (!string.IsNullOrEmpty(currentAttackAnimName))
            {
                ...
                attackTimer = 0f; // 공격 시작 후 쿨타임 다시 초기화 (원하는 동작 방식에 따라 조절 가능)
            }
        }
    }
